...
const KEY_PEN = 'celestine.penName'

/** Nom d'autrice (pseudo) utilisé dans les partages — JAMAIS le prénom réel :
 *  le prénom reste local à l'appareil (affichage), seul ce pseudo voyage. */
export function getPenName(): string | null {
  return rawGet(KEY_PEN)
}
export function setPenName(pen: string) {
  const clean = pen.trim().slice(0, 20)
  if (clean) rawSet(KEY_PEN, clean)
  else rawRemove(KEY_PEN)
}

/** Effacement TOTAL des données locales (droit à l'effacement, RGPD-enfants) :
 *  toutes les clés celestine.* + la base d'assets IA (IndexedDB). Les caches
 *  applicatifs sont purgés par l'appelant via hardReset() (pwa.ts). */
export function wipeAllData(): void {
  ...
  try {
    ...
    for (let i = 0; i < localStorage.length; i++) {
      ...
      if (k && k.startsWith('celestine.')) doomed.push(k)
    }
  ...
  try {
    indexedDB.deleteDatabase('celestine-assets')
  } catch {
    /* idem */
  }
}
